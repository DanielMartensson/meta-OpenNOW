...
a Qt 6 Quick shell with a Rust native runtime — and packages it for embedded
Linux (aarch64 / x86_64 Linux only).
...
| meta-rust-bin      | `rust-bin-layer`       | prebuilt rustc/cargo (`cargo_bin` class), Rust >= 1.85 (validated: 1.98.1) |
| meta-clang         | `clang-layer`          | bindgen needs `clang-native` |
| meta-opennow       | (this layer)           | |
...
| `opennow` | the Qt shell (`opennow-qt`), links the prebuilt runtime |
| `opennow-image` | minimal reference image |

## Usage

Add the layer and its dependencies to `bblayers.conf`, enable the distro
features above, accept the `commercial` license flag then add `opennow`
to an image:
...
* [OpenSTLinux / STM32MP2x integration (manual)](manuals/openstlinux.md)
* [Updating the OpenNOW revision and crate pins](manuals/update-sources.md)

## Development

After changing the pinned OpenNOW revision (or `Cargo.lock`), regenerate the
pins:
