...
def main():
    ...
    parser.add_argument("--opennow-root", type=Path,
                        help="Path to an OpenNOW source checkout (default: auto-detect when this "
                             "script lives inside the OpenNOW repository)")
    args = parser.parse_args()
    if args.check and args.revision:
        parser.error("--check and --revision cannot be combined")
    default_root = Path(__file__).resolve().parents[2]
    if not (default_root / "native" / "opennow-core" / "Cargo.lock").exists():
        default_root = None
