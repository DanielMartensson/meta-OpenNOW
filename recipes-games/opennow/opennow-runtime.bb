...
# cargo_bin (meta-rust-bin) provides the prebuilt rustc/cargo toolchain and
# the cross C/C++/linker wrappers.
inherit cargo_bin pkgconfig features_check nopackages opennow-cargo

COMPATIBLE_HOST = "(x86_64|aarch64).*-linux$"
REQUIRED_DISTRO_FEATURES = "opengl vulkan wayland"
DEPENDS += "alsa-lib clang-native cmake-native dbus ffmpeg libdrm nasm-native \
    opennow-license-report-native udev vulkan-headers vulkan-loader \
    wayland wayland-native wayland-protocols"

# Decode/encode backends exposed by the streamer. STM32MP25 has no VAAPI
# (no Intel GPU), so ffmpeg (software/v4l2) is the default. Override per
# machine, e.g. "linux-ffmpeg,linux-vaapi" on x86_64/aarch64 with VAAPI.
OPENNOW_RUST_FEATURES ??= "linux-ffmpeg"
...
# crate fetcher already unpacked the crates into ${CARGO_HOME}/bitbake,
# and cargo_common_do_patch_paths is an OE cargo.bbclass concept we do not
# use; sdl2 + sanity checks run afterwards.
...
do_compile() {
    opennow_cargo_build --manifest-path ${S}/native/opennow-streamer/Cargo.toml \
        --package opennow-streamer-ffi --lib --features ${OPENNOW_RUST_FEATURES}
    opennow_cargo_build --manifest-path ${S}/native/opennow-streamer/Cargo.toml \
        --package opennow-streamer --bin opennow-streamer --features ${OPENNOW_RUST_FEATURES}
...
do_install() {
    ...
    for artifact in opennow-core opennow-update-helper opennow-acceptance-verify \
        ...
        install -m 0755 ${CARGO_BINDIR}/$artifact ${D}${libdir}/opennow-native/
    done
